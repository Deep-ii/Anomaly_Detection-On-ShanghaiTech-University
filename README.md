...
Each frame is passed through YOLO to detect and crop relevant objects (persons, bicycles, vehicles). This focuses CLIP on localized regions where anomalies typically occur — rather than processing the entire background.

### 2. CLIP Feature Extraction
The cropped objects are encoded into 512-dimensional vectors using the CLIP image encoder. Simultaneously, carefully engineered text prompts describing normal and abnormal behaviors are encoded using the CLIP text encoder.

### 3. Contrastive Anomaly Scoring
For each object embedding, cosine similarity is computed against all text prompts:
```
score = max(sim(image, abnormal_prompts)) − max(sim(image, normal_prompts))
```
Frame-level anomaly score = maximum object score in that frame.

### 4. Score Calibration Pipeline
```python
# Softmax: convert scores to probabilities
P(abnormal) = softmax(similarities)[abnormal_indices].sum()

# Z-score: normalize across scenes
z = (score - mean_train) / std_train

# Gaussian smoothing: stabilize over time
smoothed = gaussian_filter1d(z_scores, sigma=5)
```

### 5. Threshold Decision
```
smoothed_score > threshold  →  🚨 ANOMALY DETECTED
smoothed_score ≤ threshold  →  ✅ NORMAL ACTIVITY
```
...
Detection accuracy is directly tied to prompt quality. The system uses a curated set of prompts refined iteratively:

**Abnormal Prompts:**
```
"a person fighting"
"a person running aggressively"
"a vehicle driving through a pedestrian area"
"a person throwing a bag"
"a person chasing another person"
"a motorbike in a pedestrian area"
"people scattering suddenly"
"a person falling down"
"an object thrown violently"
```
...
> 💡 **Key Insight:** The system only detects what is described in prompts. Expanding prompt coverage directly improves detection — no retraining required.

---

## 🖥️ Real-Time Monitoring Dashboard

Built with **Streamlit**, the monitoring interface provides:

| Panel | Contents |
|---|---|
| **Left Sidebar** | Frame folder path, Score CSV input, Anomaly threshold slider, Playback FPS control |
| **Main Panel** | Live CCTV frame playback, NORMAL / ANOMALY status indicator with score |
| **Right Panel** | Real-time anomaly score graph (updates per frame) |
